...
import { Building2, Users, ChevronRight, RefreshCw } from 'lucide-react'
...
function CompanyRow({ item }: { item: CompanyWithVendors }) {
  ...
  return (
    <button
      ...
      className="w-full flex items-center gap-4 px-5 py-4 bg-white border border-slate-100 rounded-2xl hover:border-[#128C7E]/30 hover:shadow-sm transition-all text-left"
    >
      ...
      <span className="flex items-center gap-1 bg-slate-100 text-slate-600 text-xs font-bold px-2.5 py-1 rounded-full shrink-0">
    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-bold text-slate-800 text-sm">Empresas</h2>
        </div>
        <button
          onClick={refresh}
          disabled={loading}
